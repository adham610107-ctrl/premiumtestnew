// ============================================================
// PRO EXAM v11 — Main Script
// Fixes: AutoKick, EyeComfort bug, BuyCoffee(7x,UZ,anim),
//        Certificate mobile, Timer layout, iOS notch, DB
// ============================================================
...
async function dbSave(key, value) {
    try { await fetch(DB_URL, { method:'POST', body: JSON.stringify({ action:'set', key, value: JSON.stringify(value) }) }); } catch(e){}
}
async function dbLoad(key) {
    try { const r = await fetch(DB_URL+'?action=get&key='+encodeURIComponent(key)); const j=await r.json(); if(j.value) return JSON.parse(j.value); } catch(e){}
    return null;
}
async function saveLeaderboard(user, score, subject, mode) {
    try { await fetch(DB_URL, { method:'POST', body: JSON.stringify({ action:'leaderboard', user, score, subject, mode, date: new Date().toISOString() }) }); } catch(e){}
}
async function sendLog(type, data) {
    try { await fetch(DB_URL, { method:'POST', body: JSON.stringify({ action:'log', type, data, user: localStorage.getItem('pro_exam_name')||'unknown', date: new Date().toISOString() }) }); } catch(e){}
}
...
async function idbGet(key){try{const db=await openDB();return new Promise((res,rej)=>{const tx=db.transaction('kv','readonly');const req=tx.objectStore('kv').get(key);req.onsuccess=()=>res(req.result?req.result.value:null);req.onerror=()=>rej(req.error);});}catch(e){return localStorage.getItem(key);}}
async function idbSet(key,value){try{const db=await openDB();return new Promise((res,rej)=>{const tx=db.transaction('kv','readwrite');tx.objectStore('kv').put({key,value});tx.oncomplete=()=>res(true);tx.onerror=()=>rej(tx.error);});}catch(e){localStorage.setItem(key,value);}}
...
// === SECURITY ===
function copyCard(){const el=document.getElementById("card-num")||document.getElementById("card-num-donate");const t=el?el.innerText:"9860350141282409";navigator.clipboard.writeText(t).then(()=>alert("✓ Karta raqami nusxalandi: "+t)).catch(()=>alert("Karta: "+t));}
document.addEventListener('contextmenu',e=>e.preventDefault());
document.addEventListener('keydown',function(e){if(e.keyCode===123||(e.ctrlKey&&e.shiftKey&&(e.keyCode===73||e.keyCode===74))||(e.ctrlKey&&e.keyCode===85)){e.preventDefault();return false;}if(e.ctrlKey&&e.keyCode===67){e.preventDefault();alert("⚠ Nusxalash taqiqlangan!");return false;}});
let cheatWarnings=0;
document.addEventListener("visibilitychange",()=>{const ts=document.getElementById("test-screen");if(ts&&!ts.classList.contains("hidden")&&document.hidden){cheatWarnings++;if(cheatWarnings>=3){alert("❌ 3 marta oynadan chiqdingiz. Sessiya yakunlandi!");finishExam(true);}else{alert(`⚠ OGOHLANTIRISH (${cheatWarnings}/3)\nBoshqa oynaga o'tish test sessiyasini yakunlaydi!`);}}});
...
function startBlockCheck(){
    if(blockCheckInterval)clearInterval(blockCheckInterval);
    blockCheckInterval=setInterval(()=>{const a=localStorage.getItem('pro_exam_auth');if(a==='true')checkAdminBlock();else{clearInterval(blockCheckInterval);blockCheckInterval=null;}},30000);
}
...
function playFeedback(type){
    ...
    if(type==='correct'){osc.type='sine';osc.frequency.setValueAtTime(600,audioCtx.currentTime);osc.frequency.exponentialRampToValueAtTime(1200,audioCtx.currentTime+0.12);gain.gain.setValueAtTime(0.28,audioCtx.currentTime);gain.gain.exponentialRampToValueAtTime(0.01,audioCtx.currentTime+0.12);osc.start();osc.stop(audioCtx.currentTime+0.12);if("vibrate"in navigator)navigator.vibrate(50);}
    else{osc.type='sawtooth';osc.frequency.setValueAtTime(280,audioCtx.currentTime);osc.frequency.exponentialRampToValueAtTime(90,audioCtx.currentTime+0.22);gain.gain.setValueAtTime(0.28,audioCtx.currentTime);gain.gain.exponentialRampToValueAtTime(0.01,audioCtx.currentTime+0.22);osc.start();osc.stop(audioCtx.currentTime+0.22);if("vibrate"in navigator)navigator.vibrate([150,100,150]);}
}
function createParticles(event){if(!event)return;const x=event.clientX,y=event.clientY;for(let i=0;i<14;i++){let p=document.createElement('div');p.className='magic-particle';document.body.appendChild(p);let dx=(Math.random()-0.5)*130,dy=(Math.random()-0.5)*130;p.style.left=x+'px';p.style.top=y+'px';p.animate([{transform:'translate(0,0) scale(1)',opacity:1},{transform:`translate(${dx}px,${dy}px) scale(0)`,opacity:0}],{duration:620,easing:'ease-out'});setTimeout(()=>p.remove(),620);}}
function speakQuestion(idx){if('speechSynthesis' in window){window.speechSynthesis.cancel();const msg=new SpeechSynthesisUtterance(currentTest[idx].q);msg.lang='uz-UZ';msg.rate=0.9;window.speechSynthesis.speak(msg);}else alert("Brauzer ovozli o'qishni qo'llab-quvvatlamaydi.");}
...
// === STREAK & GREETING ===
function updateDailyStreak(){const today=new Date().toDateString();const lastDate=localStorage.getItem('adham_last_date');let streak=parseInt(localStorage.getItem('adham_streak'))||0;if(lastDate!==today){const yesterday=new Date();yesterday.setDate(yesterday.getDate()-1);streak=(lastDate===yesterday.toDateString())?streak+1:1;localStorage.setItem('adham_last_date',today);localStorage.setItem('adham_streak',streak);}const e1=document.getElementById('streak-count'),e2=document.getElementById('streak-dash');if(e1)e1.innerText=streak;if(e2)e2.innerText=streak;}
function updateGreeting(){const h=new Date().getHours();let t=h>=5&&h<12?"Xayrli tong":h>=12&&h<18?"Xayrli kun":h>=18&&h<22?"Xayrli kech":"Xayrli tun";const el=document.getElementById('greeting-text');if(el)el.innerText=t;}
...
async function loadData(){
    ...
    for(const f of files){
        try{const res=await fetch(f);const data=await res.json();const subName=f.replace('.json','');
        data.forEach(q=>{let opts=q.options.filter(o=>o!==null&&o!==undefined&&o.toString().trim()!=='');let uniqueOpts=[...new Set(opts)];let correctText=q.options[q.answer];if(uniqueOpts.length===3)uniqueOpts.push("Barcha javoblar to'g'ri");bank.push({id:globalId++,subject:subName,q:q.q,originalOpts:uniqueOpts,correctText});});}
...
// === THEME & COMFORT EYE — DARK MODE BUG FIXED ===
function toggleTheme(){
    const slider=document.getElementById('theme-slider');
    if(slider&&slider.checked){
        document.body.classList.replace('light-mode','dark-mode');
        localStorage.setItem('theme','dark');
        // Fix: comfort eye incompatible with dark mode
        ...
        // Restore comfort eye if it was on
        if(localStorage.getItem('comfort_eye')==='on') applyComfortEye(true);
    }
}
function applyComfortEye(on){
    ...
    if(on&&!isDark){
        ...
        if(btn)btn.classList.remove('eye-active');
        if(ce)ce.classList.add('active-eye');if(oe)oe.classList.remove('active-eye');
        if(!on)localStorage.setItem('comfort_eye','off');
    }
}
function toggleComfortEye(){const isOn=document.body.classList.contains('comfort-eye');applyComfortEye(!isOn);if(isOn)localStorage.setItem('comfort_eye','off');}

// === SCREEN NAV ===
function switchScreen(hideId,showId){forceCloseAllModals();document.querySelectorAll('.screen').forEach(s=>{s.classList.remove('active');s.classList.add('hidden');});const el=document.getElementById(showId);if(el){el.classList.remove('hidden');el.classList.add('active');}}
function handleLogin(){
    const name=document.getElementById('student-name').value.trim();
    if(name.length<2)return alert("Ismingizni kiriting!");
    if(name.toLowerCase()==='adham')alert("Assalomu alaykum, Admin! 🔑 Maxsus rejim yoqildi.");
    currentUser=name;const dnEl=document.getElementById('display-name');if(dnEl)dnEl.innerText=name;
    if(audioCtx.state==='suspended')audioCtx.resume();
    document.getElementById('global-nav').classList.remove('hidden');
    switchScreen('welcome-screen','dashboard-screen');updateGreeting();updateProgressChart(currentChartPeriod);updateCategoryProgress();showRentgenOnDashboard();
}
function goHome(){
    ...
    switchScreen('test-screen','dashboard-screen');updateDashboardStats();updateProgressChart(currentChartPeriod);updateCategoryProgress();showRentgenOnDashboard();
}
...
function getHistoryByPeriod(period){
    const history=stats.history||[];const now=new Date();let labels=[],correctData=[],errorData=[];
...
function checkAns(qIdx,optIdx,event){
    ...
    localStorage.setItem('adham_pro_stats',JSON.stringify(stats));
    const opts=document.getElementById(`opts-${qIdx}`).getElementsByTagName('button');for(let b of opts)b.disabled=true;
...
function finishExam(force=false){
    ...
    if(!isExamMode&&correctCount<currentTest.length&&!force){
        ...
        recordHistory(correctCount,currentTest.length-correctCount);showResult(correctCount);
...
// === BUY ME COFFEE — FIXED: Uzbek, blur backdrop, magic animation ===
function showBuyCoffeeModal(){
    const modal=document.getElementById('modal-donate');if(!modal)return;
    modal.style.display='flex';
    const content=modal.querySelector('.modal-content');
    if(content){content.style.animation='none';void content.offsetWidth;content.style.animation='coffeeModalIn 0.6s cubic-bezier(0.175,0.885,0.32,1.275) forwards';}
}
function closeCoffeeModal(){
    const modal=document.getElementById('modal-donate');if(!modal)return;
    const content=modal.querySelector('.modal-content');
    // Spawn magic stars
    for(let i=0;i<20;i++)spawnMagicStar(content);
    if(content){content.style.animation='coffeeModalOut 0.7s cubic-bezier(0.68,-0.55,0.265,1.55) forwards';setTimeout(()=>{modal.style.display='none';content.style.animation='';},700);}
    else modal.style.display='none';
}
function spawnMagicStar(anchor){
    const rect=anchor?anchor.getBoundingClientRect():{left:window.innerWidth/2,top:window.innerHeight/2,width:0,height:0};
    const star=document.createElement('div');star.className='coffee-magic-star';
    star.innerText=['☕','⭐','✨','💛','🎉','🌟'][Math.floor(Math.random()*6)];
    star.style.left=(rect.left+rect.width/2)+'px';star.style.top=(rect.top+rect.height/2)+'px';
    document.body.appendChild(star);
    const angle=Math.random()*Math.PI*2,dist=80+Math.random()*120;
    const dx=Math.cos(angle)*dist,dy=Math.sin(angle)*dist;
    star.animate([{transform:'translate(0,0) scale(0.5) rotate(0deg)',opacity:1},{transform:`translate(${dx}px,${dy}px) scale(1.5) rotate(${Math.random()*360}deg)`,opacity:0}],{duration:700+Math.random()*300,easing:'ease-out'});
...
function renderRentgenBars(view){
    ...
    subjectsInTest.forEach(sub=>{
        ...
        const subPercent=Math.round((subCorrect/subQs.length)*100);const subError=100-subPercent;
...
async function downloadCertificate(){
    ...
    try{
        ...
        const canvas=await html2canvas(certEl,{
            scale:scale,useCORS:true,allowTaint:true,backgroundColor:'#0A0A0A',logging:false,imageTimeout:15000,
            onclone:(doc)=>{const cl=doc.getElementById('printable-cert');if(cl){cl.style.width='860px';cl.style.height='auto';cl.style.minHeight='608px';cl.style.position='relative';}}
        });
