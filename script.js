...
function createGreenSparkle(cx, cy) {
    ...
    for (let i = 0; i < COUNT; i++) {
        ...
        spark.style.cssText = `left:${cx}px;top:${cy}px;width:${size}px;height:${size}px;` +
            `background:radial-gradient(circle,#AFFFB0,#00C853);` +
            `box-shadow:0 0 8px 2px rgba(0,200,83,0.8);transform:translate(-50%,-50%);`;

        spark.animate([
            { transform: `translate(-50%,-50%) scale(1)`, opacity: 1 },
            { transform: `translate(calc(-50% + ${tx}px),calc(-50% + ${ty}px)) scale(0.7)`, opacity: 0.9 }
        ], { duration: 600, easing: 'cubic-bezier(0.2,0,0.8,1)', fill: 'forwards' });

        // === BOSQICH 2: MARKAZGA YIGʻILADI (600-1000ms) ===
        setTimeout(() => {
            spark.animate([
                { transform: `translate(calc(-50% + ${tx}px),calc(-50% + ${ty}px)) scale(0.7)`, opacity: 0.9 },
    ...
    // === BOSQICH 3: MARKAZDA PORSILLAYDI (1000-1200ms) ===
    // Bitta katta doira paydo bo'ladi, uchqun chiqaradi
    setTimeout(() => {
        const ring = document.createElement('div');
        ring.style.cssText = `position:fixed;left:${cx}px;top:${cy}px;width:20px;height:20px;` +
            `border-radius:50%;background:radial-gradient(circle,#AFFFB0,#00C853);` +
            `box-shadow:0 0 20px 8px rgba(0,200,83,0.9);transform:translate(-50%,-50%);` +
        ...
        ], { duration: 300, easing: 'cubic-bezier(0,0.9,0.57,1)', fill: 'forwards' });

        // BOSQICH 4: PORTLASH — barcha zarralar qayta sachraydi
        particles.forEach((spark, i) => {
            const angle2 = (Math.PI * 2 / COUNT) * i + Math.random() * 0.6;
            ...
            spark.animate([
                { transform: `translate(-50%,-50%) scale(1.4)`, opacity: 1 },
                { transform: `translate(calc(-50% + ${tx2}px),calc(-50% + ${ty2}px)) scale(0)`, opacity: 0 }
            ], { duration: 500, easing: 'cubic-bezier(0,0.9,0.57,1)', fill: 'forwards' });

            setTimeout(() => { if(spark.parentNode) spark.parentNode.removeChild(spark); }, 1520);
        });

        setTimeout(() => { if(ring.parentNode) ring.parentNode.removeChild(ring); }, 1310);
    }, 1000);
}
...
async function checkQRToken() {
    ...
    // Tokenni vaqtincha saqlaymiz — login paytida ishlatiladi
    localStorage.setItem('pending_qr_token', token);

    // Foydalanuvchi avval ism kiritishi kerak
    ...
    // Backend: 1 martalik token tekshiruvi + ro'yxatdan o'tkazish
    try {
        const r = await fetch(GOOGLE_SCRIPT_URL, {
            method: 'POST',
